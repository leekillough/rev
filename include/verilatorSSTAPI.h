...
typedef std::pair<std::string, uint64_t> PortReset;
...
enum class PortEventAction : uint8_t { WRITE = 0b00000000, READ = 0b00000001 };

class PortEvent : public SST::Event {
public:
  /// PortEvent: default constructor
  explicit PortEvent() : Event(), AtTick( 0x00ull ), action( PortEventAction::READ ) {}

  explicit PortEvent( uint64_t Tick, PortEventAction action ) : Event(), AtTick( Tick ), action( action ) {}

  explicit PortEvent( std::vector<uint8_t> P ) : Event(), AtTick( 0x00ull ), action( PortEventAction::WRITE ) {
    std::copy( P.begin(), P.end(), std::back_inserter( Packet ) );
  }

  explicit PortEvent( std::vector<uint8_t> P, uint64_t Tick ) : Event(), AtTick( Tick ), action( PortEventAction::WRITE ) {
    std::copy( P.begin(), P.end(), std::back_inserter( Packet ) );
  }
...
public:
  ...
  SST_ELI_DOCUMENT_PARAMS( { "verbose", "Set the verbosity of output for the device", "0" }, )

  /// VerilatorSSTBase: constructor
  VerilatorSSTBase( std::string DerivedName, ComponentId_t id, const Params& params );
  ...
  virtual void writePort( std::string portName, const std::vector<uint8_t>& packet )                      = 0;

  /// VerilatorSSTBase: write to the target port at the target clock cycle
  virtual void writePortAtTick( std::string portName, const std::vector<uint8_t>& packet, uint64_t tick ) = 0;
