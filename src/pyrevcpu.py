...
class RevCPU:
  ...
  def enableRevSim(self,cpus,cores,clock,memSize,machine,startAddr,memCost,program,verbose):
    ...
    for i in range(cpus):
      ...
      cpu.addParam("startAddr",startAddr)
  ...
  def build(self):
    ...
    for i in range(self.precincts):
      ...
      for j in range(self.zones):
        ...
        zqm_link.connect( (zqm_iface, "port", "1us"),
                          (zone_router, "port"+str(self.zapPerZone), "1us") )
        ...
        zen_link.connect( (zen_iface, "port", "1us"),
                          (zone_router, "port"+str(self.zapPerZone+1), "1us") )

        zen_prec_nic = zen.setSubComponent("prec_nic", "forza.zopNIC")
        zen_prec_iface = zen_nic.setSubComponent("iface", "merlin.linkcontrol")
        ...
        zen_prec_link.connect( (zen_prec_iface, "port", "1us"),
        ...
        rza_link.connect( (rza_iface, "port", "1us"),
                          (zone_router, "port"+str(self.zapPerZone+2), "1us") )
        ...
        for k in range(self.zapsPerZone):
          ...
          zap_link.connect( (zap_iface, "port", "1us"),
