...
RevBasicMemCtrl::RevBasicMemCtrl( ComponentId_t id, const Params& params ) : RevMemCtrl( id, params ) {
  ...
  max[MemParam::lrsc]        = params.find<uint32_t>( "max_lrsc", 64 );
...
bool RevBasicMemCtrl::sendAMORequest(
  ...
  switch( RevFlagAtomic( flags ) ) {
    ...
    case RevFlag::F_AMOSUB:   recordStat( MemCtrlStats::AMOSubPending   ); break;
    case RevFlag::F_AMOTHRES: recordStat( MemCtrlStats::AMOThrsPending  ); break;
    case RevFlag::F_AMOFADD:  recordStat( MemCtrlStats::AMOFAddPending  ); break;
    case RevFlag::F_AMOFSUB:  recordStat( MemCtrlStats::AMOFSubPending  ); break;
    case RevFlag::F_AMOFSUBR: recordStat( MemCtrlStats::AMOFSubrPending ); break;
    default: break;
  };
  // clang-format on
  return true;
...
bool RevBasicMemCtrl::isMemOpAvail( const RevMemOp* Op, MemParam_t& t ) const {
  ...
  switch( Op->getOp() ) {
    ...
    case MemOp::MemOpWRITE:        param = MemParam::stores;       break;
    case MemOp::MemOpFLUSH:        param = MemParam::flush;        break;
    case MemOp::MemOpREADLOCK:     param = MemParam::readlock;     break;
    case MemOp::MemOpWRITEUNLOCK:  param = MemParam::writeunlock;  break;
    case MemOp::MemOpLOADLINK:
    case MemOp::MemOpSTORECOND:    param = MemParam::lrsc;         break;
    case MemOp::MemOpCUSTOM:       param = MemParam::custom;       break;
...
bool RevBasicMemCtrl::processNextRqst( MemParam_t& t ) {
  if( rqstQ.size() == 0 ) {
...
static void ApplyForzaAMO( RevFlag flags, void* newMem, void* Rtn, T value ) {
  if( RevFlagReturn( flags ) == RevFlag::F_AMOON ) {
    newMem = Rtn;  // 'S' = S-Type (aka ON - mem unchanged, Rd gets result)
  } else if( RevFlagReturn( flags ) == RevFlag::F_AMONO ) {
    memcpy( Rtn, newMem, sizeof( T ) );  // NO -- Rd gets original memory
  }

  // Perform the atomic operation on Target
  ApplyAMO( flags, newMem, value );

  if( RevFlagReturn( flags ) == RevFlag::F_AMONN )
    memcpy( Rtn, newMem, sizeof( T ) );  // 'M' = M-Type (aka NN - both Rd and mem get result)
}
...
AMOData RevBasicMemCtrl::performAMO( RevFlag flags, uint32_t size, void* target, const void* data ) {
  ...
  if( RevFlagAtomicFloat( flags ) != RevFlag::F_NONE ) {
    switch( size ) {
    case 4: ApplyForzaAMO( flags, &newMem, target, src.f ); break;
    case 8: ApplyForzaAMO( flags, &newMem, target, src.d ); break;
    }
  } else {
    switch( size ) {
    case 1: ApplyForzaAMO( flags, &newMem, target, src.u8 ); break;
    case 2: ApplyForzaAMO( flags, &newMem, target, src.u16 ); break;
    case 4: ApplyForzaAMO( flags, &newMem, target, src.u32 ); break;
    case 8: ApplyForzaAMO( flags, &newMem, target, src.u64 ); break;
    }
  }
...
bool RevBasicMemCtrl::clockTick( Cycle_t cycle ) {
  ...
  if( num[MemParam::fence] > 0 ) {
    if( num[MemParam::read] || num[MemParam::write] || num[MemParam::lrsc] || num[MemParam::readlock] || num[MemParam::writeunlock] || num[MemParam::custom] ) {
