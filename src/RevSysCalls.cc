...
// 4003 forza_send_word( uint64_t data, bool is_control_word )
EcallStatus RevCore::ECALL_forza_send_word() {
  uint64_t data       = (uint64_t) RegFile->GetX<uint64_t>( RevReg::a0 );
  bool     is_ctrl_wd = (bool) RegFile->GetX<bool>( RevReg::a1 );
...
// 4017, forza_spawn_word
// using this to test out the spawn plumbing
EcallStatus RevCore::ECALL_forza_spawn_word() {
  ...
  output->verbose(
    ...
    2,
    0,
    "ECALL: forza_spawn_word called by thread %" PRIu32 " on hart %" PRIu32 ", val=%" PRIu32 "\n",
    GetActiveThreadID(),
    HartToExecID,
    zNic->getPrecinctID()
  );
