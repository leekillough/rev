...
ARCH=rv64g_zicntr REV_EXE=bug-rdtime.rv64.exe sst --add-lib-path=../../build/src ./rev-test-tracer.py > bug-rdtime.rv64.log && echo passed for rv64 || exit 3
...
ARCH=rv32g_zicntr REV_EXE=bug-rdtime.rv32.exe sst --add-lib-path=../../build/src ./rev-test-tracer.py > bug-rdtime.rv32.log && echo passed for rv32 || exit 12
