...
int main() {
  ...
  // This is for one zap per zone, two zones
  ...
  {
    char msg[50] = "\ndebug:ZAPX: main: n_zaps=X, n_local_pes=Y\n";
    msg[26]      = '0' + n_zaps;
    msg[41]      = '0' + n_local_pes;
    rev_write( STDOUT_FILENO, msg, sizeof( msg ) );
  }

  // Create local actor threads (PEs)
  // Currently function call, can also use forza_thread_create
  struct ForzaThreadArgs_ FArgs[4];  // one for each PE
  ...
  for( int j = 0; j < PE_PER_ZAP; j++ ) {  // Each thread at a zap will create the local thread(s)
