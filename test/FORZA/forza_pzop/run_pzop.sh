...
if [[ -f forza_pzop.exe ]]; then
  sst --add-lib-path=../../build/src/ ../forza-test-config-ring.py -- -a 4 -z 1 -o 2 -p 1 -r ./forza_pzop.exe -s 1,1:1
else
  echo "Test FORZA forza_pzop.c: forza_send.exe not Found - likely build failed"
  exit 1
fi
