#ifndef _forza_util_h_
#define _forza_util_h_
...
static void forza_putc(fd_t fd, char c)
{
    // putc(c, stderr);
...
static void forza_puts(fd_t fd, char * s) 
{
    char c;
    if (!s)
    {
...
static void printfloat(fd_t fd, double f)//, int precision), can add precision back later
{
    char buf [40];
	char * ptr = buf;
	char * p = ptr;
...
static void forza_fprintf(fd_t fd, const char * fmt, volatile void **args) {
    char c;
    size_t i;
    int state = 0;
