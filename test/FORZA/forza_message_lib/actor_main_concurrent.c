...

{
  ...
  for (long i=0; i<fargs->n_sends; i++) {
    ...
    if (sendstat != SUCCESS) {
			char msg[60] = "\ndebug:PEX: actor thread init: REQUEST msg send C failed\n";
    	msg[9] = '0' + fargs->local_pe;
			msg[48] = '0' + i;
			rev_write(STDOUT_FILENO, msg, sizeof(msg));
		} else {
			char msg[65] = "\ndebug:PEX: actor thread init: REQUEST msg send C succeeded\n";
      msg[9] = '0' + fargs->local_pe;
			msg[48] = '0' + i;
			rev_write(STDOUT_FILENO, msg, sizeof(msg));
		}
...

{
  ...
  do {
    ...
    if (ret == SUCCESS) {
      // process message
			char msg[60] = "\ndebug:PEX: actor thread init: received REQUEST msg C: #R\n";
      msg[9] = '0' + fargs->local_pe;
			msg[52] = '0' + rcv_pkt.v2;
			msg[56] = '0' + recv_count;
			rev_write(STDOUT_FILENO, msg, sizeof(msg));
      ...
      if (recv_count != fargs->n_sends) {
        char msg[70] = "\ndebug:PEX: actor thread init: bad REQUEST DONE message count: #R\n";
        msg[9] = '0' + fargs->local_pe;
				msg[64] = '0' + recv_count;
				rev_write(STDOUT_FILENO, msg, sizeof(msg));
        assert(0);
