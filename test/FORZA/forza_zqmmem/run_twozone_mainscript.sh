...
#Build the test
make -f Makefile_twozone clean && make -f Makefile_twozone
...
if [[ -f forza_send.exe ]]; then
  sst --add-lib-path=../../build/src/ ../forza-test-config-ring.py -- -a 32 -z 1 -o 2 -p 1 -r ./forza_send_twozone_onezap.exe -s 1,1:1
