...
int main( int argc, char** argv ) {
  ...
  register uint64_t*
    addr;  // you must use register, otherwise it stucks at the sd instruction generated by compiler and forza_send won't run
  ...
  if( a == 0 ) {
    forza_send( (uint64_t) addr, 2 * sizeof( uint64_t ), 0 );
  }
