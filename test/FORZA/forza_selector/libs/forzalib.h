...
void jaccard_phase2_recv(void *pkt, int AID)
{
    ...
    if (search_ != -1) {
        ...
        int64_t search__ = binary_search(intersection_mat_->loffset[pkg2->pos_row], intersection_mat_->loffset[pkg2->pos_row + 1] - 1, pkg2->pos_col, mat_, 1);
    ...
    while(1)
    {
        ...
        for(int i = 0; i < THREADS; i++)
        {
            ...
            while(mb_request[mythread][i].pkt[recvcnt].valid)
            {
                ...
                {
                    case REQUEST:
                        jaccard_phase2_recv(mb_request[mythread][i].pkt[recvcnt].pkt, mythread);
                        break;
                    case RESPONSE:
                        break;
                    case FORZA_DONE:
