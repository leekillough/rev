...
def buildTopo():
    ...
    def fattree_rb(level, group, links):
        ...
        if level == 0:
            ...
            # Add links
            for lnk in range(len(host_links)):
                rtr.addLink(host_links[lnk], "port%d" % lnk, link_latency)
            for lnk in range(len(links)):
                rtr.addLink(links[lnk], "port%d" % (lnk+downs[0]), link_latency)
            return
        ...
        for i in range(rtrs_in_group):
            ...
            topology.addParams({"shape": shape})
            # Add links
            for lnk in range(len(rtr_links[i])):
                rtr.addLink(rtr_links[i][lnk], "port%d" % lnk, link_latency)
