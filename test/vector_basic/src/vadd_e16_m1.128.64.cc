...
void vadd_array( elem_t a[], elem_t b[], elem_t c[] ) {
  ...
  unsigned time0, time1, inst0, inst1;
  int      rc    = 0x99;
  unsigned ITERS = 4;  // AVL/VLMAX = 32/8
  elem_t*  pa    = &( a[0] );
  elem_t*  pb    = &( b[0] );
  elem_t*  pc    = &( c[0] );
  RDTIME( time0 );
  ...
  if( rc ) {
    printf( "Error: vadd_e16_m1.128.64 rc=%d\n", rc );
    assert( 0 );
  }
...
#ifndef USE_SPIKE
  printf( "[vadd_e16_m1.128.64 rev cycles] scalar=%d vector=%d\n", counters_scalar[0], counters_vector[0] );
  printf( "[vadd_e16_m1.128.64 rev instrs] scalar=%d vector=%d\n", counters_scalar[1], counters_vector[1] );
#else
  printf( "[vadd_e16_m1.128.64 spike cycles] scalar=%d vector=%d\n", counters_scalar[0], counters_vector[0] );
  printf( "[vadd_e16_m1.128.64 spike instrs] scalar=%d vector=%d\n", counters_scalar[1], counters_vector[1] );
#endif
}
