...
const unsigned   VLMAX      = 32;   // Max vector length LMUL * VLEN / SEW
const unsigned   AVL        = 127;  // Application Vector Length (elements)
//const unsigned   VL         = 32;    // Elements operated on by a vector instruction (<=VLMAX, <=AVL)
...
elem_t s0[AVL + 1]          = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10,
...
elem_t s1[AVL + 1] = { 0x0100, 0x0200, 0x0300, 0x0400, 0x0500, 0x0600, 0x0700, 0x0800, 0x0900, 0x0a00, 0x0b00, 0x0c00, 0x0d00,
...
void vadd_array( elem_t a[], elem_t b[], elem_t c[] ) {
  ...
  asm volatile( "add  a0, zero, %1  \n\t"  // Load VL
                ...
                //
                "vle32.v v0, (a1)     \n\t"   // Get first vector (v[0:7])
                "sub a0, a0, t0       \n\t"   // Decrement N
                "slli t0, t0, 2       \n\t"   // Divide by number of bytes per element
                "add a1, a1, t0       \n\t"   // Bump pointer to a
                "vle32.v v8, (a2)     \n\t"   // Get second vector (v[8:15])
                "add a2, a2, t0       \n\t"   // Bump pointer to b
                "vadd.vv v16, v0, v8   \n\t"  // Sum Vectors (v[16:23])
                "vse32.v v16, (a3)     \n\t"  // Store result(v[16:23])
                "add a3, a3, t0       \n\t"   // Bump pointer to c
  ...
  if( rc ) {
    printf( "Error: vadd_e32_m8.128.64 rc=%d\n", rc );
    assert( 0 );
  }
...
#ifndef USE_SPIKE
  printf( "[vadd_e32_m8.128.64 rev cycles] scalar=%d vector=%d\n", counters_scalar[0], counters_vector[0] );
  printf( "[vadd_e32_m8.128.64 rev instrs] scalar=%d vector=%d\n", counters_scalar[1], counters_vector[1] );
#else
  printf( "[vadd_e32_m8.128.64 spike cycles] scalar=%d vector=%d\n", counters_scalar[0], counters_vector[0] );
  printf( "[vadd_e32_m8.128.64 spike instrs] scalar=%d vector=%d\n", counters_scalar[1], counters_vector[1] );
#endif
}
