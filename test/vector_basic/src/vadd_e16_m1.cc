...
const unsigned   VL         = 32;   // Vector Length
...
elem_t s1[VL]               = { 0x0100, 0x0200, 0x0300, 0x0400, 0x0500, 0x0600, 0x0700, 0x0800, 0x0900, 0x0a00, 0x0b00,
...
void vadd_array( elem_t a[], elem_t b[], elem_t c[] ) {
  ...
  asm volatile( "add  a0, zero, %1  \n\t"  // Load VL
                ...
                "_vadd_loop: \n\t"
                //          VL=n, SEW=16b, LMUL=1, tail/mask agnostic
...
#ifndef USE_SPIKE
  printf( "[vadd_e16_m1 rev cycles] scalar=%d vector=%d\n", counters_scalar[0], counters_vector[0] );
  printf( "[vadd_e16_m1 rev instrs] scalar=%d vector=%d\n", counters_scalar[1], counters_vector[1] );
#else
  printf( "[vadd_e16_m1 spike cycles] scalar=%d vector=%d\n", counters_scalar[0], counters_vector[0] );
  printf( "[vadd_e16_m1 spike instrs] scalar=%d vector=%d\n", counters_scalar[1], counters_vector[1] );
#endif
}
