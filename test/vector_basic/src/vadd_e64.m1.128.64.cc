...
void vadd_array( elem_t a[], elem_t b[], elem_t c[] ) {
  ...
  if( rc ) {
    printf( "Error: vadd_e64_m1.128.64 rc=%d\n", rc );
    assert( 0 );
  }
...
#ifndef USE_SPIKE
  printf( "[vadd_e64_m1.128.64 rev cycles] scalar=%d vector=%d\n", counters_scalar[0], counters_vector[0] );
  printf( "[vadd_e64_m1.128.64 rev instrs] scalar=%d vector=%d\n", counters_scalar[1], counters_vector[1] );
#else
  printf( "[vadd_e64_m1.128.64 spike cycles] scalar=%d vector=%d\n", counters_scalar[0], counters_vector[0] );
  printf( "[vadd_e64_m1.128.64 spike instrs] scalar=%d vector=%d\n", counters_scalar[1], counters_vector[1] );
#endif
}
