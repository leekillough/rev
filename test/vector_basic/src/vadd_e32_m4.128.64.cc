...
const unsigned   LMUL       = 4;    // Length multiplier
...
elem_t s1[AVL] = { 0x0100, 0x0200, 0x0300, 0x0400, 0x0500, 0x0600, 0x0700, 0x0800, 0x0900, 0x0a00, 0x0b00, 0x0c00, 0x0d00,
                   ...
                   0x3500, 0x3600, 0x3700, 0x3800, 0x3900, 0x3a00, 0x3b00, 0x3c00, 0x3d00, 0x3e00, 0x3f00, 0x4000 };
...
void vadd_array( elem_t a[], elem_t b[], elem_t c[] ) {
  ...
  if( rc ) {
    printf( "Error: vadd_e32_m4.128.64 rc=%d\n", rc );
    assert( 0 );
  }
...
#ifndef USE_SPIKE
  printf( "[vadd_e32_m4.128.64 rev cycles] scalar=%d vector=%d\n", counters_scalar[0], counters_vector[0] );
  printf( "[vadd_e32_m4.128.64 rev instrs] scalar=%d vector=%d\n", counters_scalar[1], counters_vector[1] );
#else
  printf( "[vadd_e32_m4.128.64 spike cycles] scalar=%d vector=%d\n", counters_scalar[0], counters_vector[0] );
  printf( "[vadd_e32_m4.128.64 spike instrs] scalar=%d vector=%d\n", counters_scalar[1], counters_vector[1] );
#endif
}
