...
bool testSize() {
    std::vector<std::pair<int64_t, int64_t>, Allocator<std::pair<int64_t, int64_t>>> vec = {{1, 2}, {3, 4}};
...
bool testReserve() {
    ...
    vec.reserve(N);
    if ( vec.capacity() != N ) { return false; }
...
bool testBeginEnd() {
    std::vector<std::pair<int64_t, int64_t>, Allocator<std::pair<int64_t, int64_t>>> vec = {{1, 1}, {2, 2}, {3, 3}};
    auto it = vec.begin();
    if (it == vec.end() || it->first != 1 || it->second != 1) return false;
