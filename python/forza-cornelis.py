                ...
                for port in range(0,max_port+1):
                    ...
                    if not portDf.empty: 
                        if str(portDf[portDf.port == port].head().type1.values[0]) == "FI":
                            ...
                            linkName = "link_{guid}_{port}_{guid1}_{port1}".format(guid=portDf[portDf.port == port].head().guid.values[0], port=portDf[portDf.port == port].head().port.values[0],  guid1=portDf[portDf.port == port].head().guid1.values[0], port1=portDf[portDf.port == port].head().port1.values[0])
                            linkName2 = "link_{guid1}_{port1}_{guid}_{port}".format(guid=portDf[portDf.port == port].head().guid.values[0], port=portDf[portDf.port == port].head().port.values[0],  guid1=portDf[portDf.port == port].head().guid1.values[0], port1=portDf[portDf.port == port].head().port1.values[0])
...
if __name__ == "__main__":
  ...
  ap.add_argument("-t", "--maxTerminal", required=True, help="Sets the Max terminal for the switch for the router configurations")
