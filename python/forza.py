...
def buildTopo():
    ...
    def fattree_rb(level, group, links):
        ...
        if level == 0:
            ...
            # Add links
            for ln in range(len(host_links)):
                rtr.addLink(host_links[ln], "port%d" % ln, link_latency)
            for ln in range(len(links)):
                rtr.addLink(links[ln], "port%d" % (ln + downs[0]), link_latency)
            return
        ...
        for i in range(rtrs_in_group):
            ...
            topology.addParams({"shape": shape})
            # Add links
            for ln in range(len(rtr_links[i])):
                rtr.addLink(rtr_links[i][ln], "port%d" % ln, link_latency)
